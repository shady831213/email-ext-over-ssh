...
import org.jenkinsci.plugins.emailextoverssh.plugins.CssInliner;
...
import org.jenkinsci.plugins.emailextoverssh.plugins.content.AbstractEvalContent;
import org.jenkinsci.plugins.emailextoverssh.plugins.content.EmailExtScript;
import org.jenkinsci.plugins.emailextoverssh.plugins.content.TriggerNameContent;
import org.jenkinsci.plugins.emailextoverssh.watching.EmailExtWatchAction;
import org.jenkinsci.plugins.emailextoverssh.watching.EmailExtWatchJobProperty;
...
public class ExtendedEmailPublisher extends Notifier implements MatrixAggregatable {
    ...
    public boolean prebuild(AbstractBuild<?, ?> build, BuildListener listener) {
        ...
        if (!(build instanceof MatrixRun) || isExecuteOnMatrixNodes()) {
            ...
            try {
                ...
                listener.getLogger().println("[SSH] Exception:" + e.getMessage());
    ...
    private boolean _perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener, boolean forPreBuild) throws InterruptedException {
        if (disabled) {
        ...
        for (String triggerName : triggered.keySet()) {
            for (EmailTrigger trigger : triggered.get(triggerName)) {
                ...
                //sendMail(context);
                sendMailOverSSH(context, listener.getLogger());
            }
        }
    ...
    private MimeMessage createMail(ExtendedEmailPublisherContext context) throws MessagingException, IOException, InterruptedException {
        ...
        if (descriptor.getFrom() != null) {
            fromAddress = new InternetAddress(descriptor.getFrom());
        }
     ...
     */
    @SuppressFBWarnings("REC_CATCH_EXCEPTION")
    boolean sendMailOverSSH(ExtendedEmailPublisherContext context, PrintStream logger) throws InterruptedException {
        final ExtendedEmailPublisherDescriptor descriptor = getDescriptor();
        final CredentialsSSHSite site = new CredentialsSSHSite(descriptor.getSshHostname(),
                descriptor.getSshPort(), descriptor.getCredentialId(), descriptor.getSshServerAliveInterval(),
                descriptor.getSshTimeout(), descriptor.getSshPty());
        com.jcraft.jsch.Session session = null;
        try {
            session = site.createSession(logger);
            int assinged_port = session.setPortForwardingL(Integer.parseInt(descriptor.getSshForwardPort()),
                    descriptor.getSmtpServer(), Integer.parseInt(descriptor.getSmtpPort()));
            logger.println(descriptor.getSshHostname()+assinged_port+" -> "+descriptor.getSmtpServer()+":"+descriptor.getSmtpPort());
